...
提问「四君子汤由哪些中药组成？」，Agent 经查询理解 → 三路检索 → RRF 融合 → 精排 → 自反思 → 安全兜底后，SSE 流式输出答案并附带文献来源。

![Agentic RAG 问答演示](./assets/demo-qa.gif)

### 登录页

iOS 液态玻璃（Liquid Glass）登录卡片：`backdrop-filter` 强模糊 + 增饱和让背景光斑透过呈现流动折射，渐变折射描边模拟玻璃厚边，左上角镜面高光扫光做表面反光膜。背景由三个异步运动的渐变光斑（`blur(70px)` + 12/15/18s 漂浮周期）组成，整体简约高级。左栏深绿玻璃品牌区 + 右栏白纱磨砂表单区，支持自助注册与演示账号一键填充。

![登录页](./assets/login.png)

### 辨证问答（Agentic RAG 核心）

用户提问后，Agent 经查询理解 → 三路检索（向量 + BM25 + 图谱）→ RRF 融合 → 精排 → 自反思 → 安全兜底 → 流式生成。回答附带文献来源与图谱路径，SSE 实时推送检索步骤与反思轨迹。

![辨证问答 - RAG 回答](./assets/qa-answer.png)

### 运行概览

14 日问答趋势、角色分布、知识主题分布、文档状态分布、检索质量（兜底率 / 满意度）实时聚合，数据全部来自后端接口真实落库。

![运行概览](./assets/dashboard.png)

### 典籍知识库

多格式文档（PDF / Markdown / Word / Excel / PPT）批量上传、自动解析切片向量化；支持下载、重命名、删除与详情查看。

![典籍知识库](./assets/library.png)

### 本草图谱

Neo4j 知识图谱可视化浏览。点击左侧实体（如「四君子汤」），中间力导向图渲染该实体的 2 跳邻居关系网络（方剂 → 中药 → 证候 → 功效 等多类型节点以不同颜色区分），右侧面板展示实体别名、说明与来源；悬停连线可查看完整三元组关系。

![本草图谱 - 四君子汤实体关系图](./assets/graph.png)
