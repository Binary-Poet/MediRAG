...
class Settings(BaseSettings):
    ...
    # 覆盖度阈值：各子查询自己的精排最高分过线才算「这个实体有依据」。与上面的拒答阈值分开——
    # 拒答看全局 max（证据够不够强），覆盖度看逐子查询（几个实体真查到了），调一个不该动另一个。
    # 0.6 为实测取值：8 道比较题 / 16 个子查询，库内确有其内容的子查询落在 0.854~0.992
    # （含白术这种只在四君子汤组成里出现的），库里没有或仅被顺带提及的落在 0.078~0.445
    # （党参 0.44 / 西洋参 0.37 / 柴胡 0.27——柴胡仅在补中益气汤组成里列名，无疏肝内容）。
    # 中间 0.445~0.854 是空档，0.6 距两侧各留 0.15 以上余量。演示库仅 13 切片，样本有限，
    # 换语料后应重新实测校准。
    coverage_min_score: float = 0.6
    graph_hop: int = 2          # 图谱检索跳数上限（激活「证候→方剂→组成」类多跳链式检索）
